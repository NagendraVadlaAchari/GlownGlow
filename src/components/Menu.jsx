...
export default function Menu() {
  ...
  /* ── Email via Web3Forms API (real email, no mail-client popup) ── */
  const handleCartEmail = async () => {
    ...
    try {
      const res = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          subject: '🛒 New Cart Order - Grow n Glow',
  ...
  /* ── WhatsApp via native app (no web redirect) ── */
  const handleCartWhatsApp = () => {
    const text =
      `🛒 *New Cart Order — Grow n Glow*\n\n` +
      buildCartLines() +
      ...
      {totalItems > 0 && (
        <div className="cart-float-group">
          {cartSent ? (
            ...
            <>
              <button className="cart-float-btn" id="cart-email-btn" onClick={handleCartEmail} title="Send order via Email" disabled={emailLoading}>
                {emailLoading ? (
              ...
              <button className="cart-float-btn cart-float-btn--wa" id="cart-whatsapp-btn" onClick={handleCartWhatsApp} title="Send order via WhatsApp">
